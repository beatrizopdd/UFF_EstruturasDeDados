...
int ehNaoOrientado(TGrafo *g) {
    TGrafo *saidaA = g, *saidaB;
    TVizinho *chegadaA, *chegadaB;

    while (saidaA != NULL) {
        chegadaA = saidaA->prim_vizinho;
        while (chegadaA != NULL) {
            saidaB = busca_vertice(g, chegadaA->id_vizinho);
            chegadaB = busca_vizinho(saidaB->prim_vizinho, saidaA->id_vertice);
            if ((chegadaB == NULL) || (chegadaA->peso != chegadaB->peso))
                return 0;
            chegadaA = chegadaA->prox;
        }
        saidaA = saidaA->prox;
    }
    return 1;
